...
export default function AdminMarketingPage() {
  ...
  const handleExport = () => {
    ...
    const rows = employees.map(e => [
      e.name, e.position, e.email || "", e.phone || "",
      e.salary ?? "",
    ...
    a.href = url;
    a.download = `marketing-employees-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
  ...
  const handleDelete = async () => {
    try {
      setSubmitting(true);
      await marketingService.deleteEmployee(selected!._id);
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight leading-none">
              {t("marketing")} <span className="text-emerald-400">{t("employees")}</span>
            </h1>
