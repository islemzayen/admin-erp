...
import {
  Megaphone, Search, Plus, Download,
...
const POSITIONS = ["Employee", "Marketing Manager"];
...
export default function AdminMarketingPage() {
  ...
  const fetchAll = async () => {
    try {
      setLoading(true);
      const [emps, st] = await Promise.all([marketingService.getAllEmployees(), marketingService.getStats()]);
  ...
  const handleExport = () => {
    ...
    a.href=url; a.download=`marketing-employees-${new Date().toISOString().split("T")[0]}.csv`; a.click(); URL.revokeObjectURL(url);
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const result = await marketingService.createEmployee({ ...form, salary: Number(form.salary)||0 });
  ...
  const handleEdit = async () => {
    ...
    try {
      setSubmitting(true); setFormError("");
      await marketingService.updateEmployee(selected!._id, { ...form, salary: Number(form.salary)||0 });
  ...
  const handleDelete = async () => {
    try { setSubmitting(true); await marketingService.deleteEmployee(selected!._id); await fetchAll(); setShowDelete(false); }
    ...
    <DashboardLayout>
      <div className="min-h-screen bg-gray-100 dark:bg-[#060a0f] text-gray-900 dark:text-white font-mono p-6 space-y-6 transition-colors duration-300">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight leading-none">{t("marketing")} <span className="text-emerald-400">{t("employees")}</span></h1>
            <p className="text-xs text-gray-500 mt-1.5 uppercase tracking-widest">{t("marketingDept")}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            { label: t("totalMarketing"), value: String(stats.total), sub: t("inDepartment"), icon: <Megaphone size={14} />, iconBg: "bg-blue-500/10 text-blue-400" },
        ...
        <div className={`${card} overflow-hidden`}>
          <div className="flex flex-wrap items-center justify-between gap-3 px-6 py-5 border-b border-gray-200 dark:border-white/[0.05]">
            <div>
              <h2 className="text-base font-bold">Marketing Team</h2>
